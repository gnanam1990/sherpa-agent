# Product Brief: Sherpa Agent

## One-Line Pitch

Sherpa Agent gives AI agents USDC budgets with contract-enforced daily caps, counterparty limits, and on-chain audit logs.
...
Sherpa Agent moves spend control into a smart contract. The operator deploys a `SpendAccount`, funds it with USDC, sets limits, and gives the agent only a spender key. Every request is checked by the contract before funds move.
