...
Sherpa Agent is a corporate-card-style spend manager for agents. The
...
```bash
pnpm --filter @sherpa-agent/demo-agent start -- --dry-run
```
...
```bash
pnpm --filter @sherpa-agent/dashboard dev
```
...
pnpm testnet:status
pnpm contracts:test
pnpm --filter @sherpa-agent/guardrails test
pnpm --filter @sherpa-agent/demo-agent start -- --dry-run
pnpm --filter @sherpa-agent/dashboard dev
```
