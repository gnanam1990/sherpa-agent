...
Sherpa Agent is a smart-contract-backed spend manager that gives any AI
...
| SDK | Gives agents a drop-in spend client | `@sherpa-agent/guardrails` |
| Intent | Turns plain-English payment requests into guarded spend attempts | `@sherpa-agent/intent` |
| Policy API | Previews policy and simulations off-chain | `apps/api`, `@sherpa-agent/policy` |
| Integrations | Guards x402-style payment requirements | `@sherpa-agent/x402` |
...
flowchart LR
  ...
  Agent["AI Agent"] -->|"plain-English payment intent"| Intent["@sherpa-agent/intent"]
  Intent -->|"parsed spend attempt"| SDK["@sherpa-agent/guardrails SDK"]
  X402["x402 Payment Requirement"] -->|"pre-payment policy check"| X402Pkg["@sherpa-agent/x402"]
  API["Sherpa API"] -->|"simulate / evaluate"| Policy["@sherpa-agent/policy + audit + simulator"]
  SDK --> Contract
  X402Pkg --> SDK
...
```ts
import { createGuardrailsClient } from "@sherpa-agent/guardrails";
...
```text
https://sherpa-agent-j3x8jgrop-gnanam1990s-projects.vercel.app
```
